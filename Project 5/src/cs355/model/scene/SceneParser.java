...
public class SceneParser {
	...
	/**
	 * Parse the file and build a scene from it. This method will use the
	 * ObjParser to open and parse .obj files, which are how we store models.
	 * 
	...
	/**
	 * Parse a rotation (angle and vector) from one line of text.
	 * 
	 * @param line
	 *            = the line of text to parse.
	 * @return the angle to rotate as a double. The rotation vector is
	 *         implicitly [0, 1, 0].
	 */
	 ...
	 * 
	 * @param line
	 *            = the line of text to parse. We only need the filename.
	 * @return the WireFrame parsed from the file.
	 * @throws Exception
	 *             if the .obj file is invalid.
	 */
	...
	/**
	 * Parse a camera position and orientation from some lines of text. This
	 * will continue parsing more lines, not just its own. That's because the
	 * eye has position <i>and</i> rotation.
	 * 
	 * @param line
	 *            = the first line of text to parse.
	 * @param s
	 *            = the scanner (we may need more lines).
	...
	/**
	 * Parse an instance from some lines of text. This will continue parsing
	 * more lines, not just its own. That's because an instance has position,
	 * rotation, color, a model, and scale.
	 * 
	 * @param line
	 *            = the first line of text to parse.
	 * @param s
	 *            = the Scanner with more input.
	 * @return an InstParse that we'll use later to resolve the reference to a
	 *         WireFrame.
	 */
	...
	/**
	 * Figures out if the next line of input is valid for the current parse
	 * situation. It does so <i>without</i> advancing the Scanner, which is very
	 * important.
	 * 
	 * @param s
	 *            = the Scanner with the input.
	 * @param options
	 *            = the valid options.
	 * @return true if the next line has valid input, false otherwise.
	 */
	...
	/**
	 * Small class that lets us resolve references to WireFrames at the end of
	 * parsing.
	 */
