...
/**
 * A set of logic for redrawing the screen. Underneath the hood, this is using
 * double buffering and timing to draw as predictably as possible.
 * 
 * @author Talonos
 */
class RedrawRoutine implements Runnable {
	 ...
	 * @param v
	 *            = the student's ViewRefresher that will control drawing.
	 */
	...
	public void run() {
		...
		while (true) {
			try {
				// Make sure that if all else fails, you sleep for at least a
				// millisecond
	...
	/**
	 * Refreshes the view. It does this by calling the refresh method on the
	 * provided ViewRefresher.
	 * 
	 * @return the BufferedImage that the Graphics2d drew on. Used for the
	 *         "Save Picture" option.
	 */
