...
public class StudentLWJGLController implements CS355LWJGLController {
	...
	public void updateKeyboard() {
		...
		if (Keyboard.isKeyDown(Keyboard.KEY_O)) {
			 ...
			 * currentlyPerspective=false; animate(true); }
			 */
			glMatrixMode(GL_PROJECTION);
			glLoadIdentity();
	...
	public void render() {
		...
		// for each house in the neighboorhood
		for (modelTrans thisModel : neighboorhood) {
			...
			glTranslatef(camera.x, camera.y, camera.z); // apply Camera
														// Translation
			glTranslatef(thisModel.x, thisModel.y, thisModel.z); // apply model
																	// translation
	...
	//generate worldSpace offsets for each house in neighborhood 
	...
	private class modelTrans {
		...
		public String toString() {
			return "Position: x=" + (int) x + " y=" + (int) y + " z=" + (int) z + " R=" + (int) r;
		}
	}
